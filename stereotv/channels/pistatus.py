...
def collect(audio=None, now=None, fps: float = 0.0) -> dict:
    s: dict = {}
    t = _vc("measure_temp")
    s["temp"] = t.split("=")[1] if "=" in t else _read("/sys/class/thermal/thermal_zone0/temp")[:2] + ".0'C"
    c = _vc("measure_clock arm")
    s["arm_mhz"] = int(c.split("=")[1]) // 1_000_000 if "=" in c else 0
...
class PiStatusChannel(Channel):
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        y = bar.bottom + 14
        rows = [
            ("CPU TEMP", s["temp"].replace("'C", "°C")),
            ("ARM CLOCK", f"{s['arm_mhz']} MHz" + ("" if s["arm_mhz"] >= 1200 else "  ← throttled")),
            ("CORE VOLT", s["core_v"]),
            ("LOAD", "  ".join(f"{v:.2f}" for v in s["load"])),
            ("MEMORY", f"{s['mem_used_mb']} / {s['mem_total_mb']} MB"),
            ("DISK", f"{s['disk_used_gb']:.1f} / {s['disk_total_gb']:.1f} GB"),
            ("WI-FI", s["wifi"] or "wired"),
            ("UPTIME", s["uptime"]),
